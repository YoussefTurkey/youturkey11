...
// Importing Data
import { works } from "@/app/database/data";
...
const Works = () => {
  ...
  const [randomWorks, setRandomWorks] = useState<typeof works>([]);

  useEffect(() => {
    // shuffle once only on client
    setRandomWorks(shuffleArray(works).slice(0, 4));
  }, []);
    ...
    <section id="projects" className="container mx-auto my-20 lg:my-50 px-10">
      ...
      <div className="flex flex-col md:flex-row items-start justify-between py-10">
        ...
        <p className="w-full md:w-90 lg:w-100 xl:w-200 lg:text-xl text-[hsl(var(--third))]">
          {language === "en"
            ? "A mix of my technical projects, artistic work, articles, and visual content. All crafted with structure, clarity, and intention."
            : "مزيج من مشاريعي التقنية، وأعمالي الفنية، ومقالاتي، ومحتواي المرئي. كلها مصممة بهيكل واضح، ووضوح، وقصدية."}
        </p>
      </div>
            ...
            <div className="flex items-start justify-between pt-5">
              <div>
                ...
                <p className="text-[hsl(var(--third))] py-1 group-hover:text-[hsl(var(--foreground))] transition-all">
                  {language === "en" ? work.shortDescEn : work.shortDescAr}
                </p>
              </div>
      ...
      <Btns style="py-3 xl:px-10 my-10 text-xl sm:text-2xl lg:text-3xl border border-[hsl(var(--secondary))] bg-transparent hover:bg-[hsl(var(--secondary))] hover:text-white flex items-center justify-center mx-auto gap-2">
